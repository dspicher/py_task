...
def simulate((repetition_i, p)):
    ...
    neuron["g_adap_delta"] = group_size*p["g_adap_delta"]
    ...
    NoI = NeuronGroup(2*group_size, nrn_eqs, threshold='phi(U, alpha, beta, r_max)*dt>rand()',
                      refractory=0.01*ms, reset='g_adap += g_adap_delta/(tau_adap/ms)', method='euler')
    NoI.U = -70.0 * mV
    NoI.V = -70.0 * mV

    eqs_syn_som_exc = '''
    #Input with weight
    dg_E_S/dt = -g_E_S/tau_s_exc : siemens (clock-driven)
    ...
    s_som_e = Synapses(NoI, NoI, eqs_syn_som_exc, on_pre="g_E_S += w/(tau_s_exc/ms)*msiemens", method='euler')
    ...
    eqs_syn_som_inh = '''
    #Input with weight
    dg_I_S/dt = -g_I_S/tau_s_inh : siemens (clock-driven)
    ...
    s_som_i = Synapses(NoI, NoI, eqs_syn_som_inh, on_pre="g_I_S += w/(tau_s_inh/ms)*msiemens", method='euler')
    ...
    ddelta/dt = ((phi(U_post, alpha, beta, r_max) - phi(V_star, alpha, beta, r_max)) * phi_prime(V_star, alpha, beta, r_max) / phi(V_star, alpha, beta, r_max) * dV_star_dw - delta)/tau_delta : 1 (clock-driven)
    ...
    for i in range(N_input/2):
        for j in range(group_size):
            plot(SynMon.t/ms,ws[j,i,:], 'b', alpha=0.1)
        for j in range(group_size, 2*group_size):
            plot(SynMon.t/ms,ws[j,i,:], 'g', alpha=0.1)
    plot(SynMon.t/ms, np.mean(ws[:group_size,:N_input/2,:],(0,1)), 'b', lw=3)
    plot(SynMon.t/ms, np.mean(ws[group_size:,:N_input/2,:],(0,1)), 'g', lw=3)
