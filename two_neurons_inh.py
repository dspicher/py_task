...
def simulate((repetition_i, p)):
    ...
    for i, ps in enumerate(pattern_starts):
        spike_counts[i, 0] = 1000/(cycle/ms)*1.0/(group_size)*np.sum(np.logical_and(spikes1 >= ps, spikes1 < ps+cycle/ms))
        spike_counts[i, 1] = 1000/(cycle/ms)*1.0/(group_size)*np.sum(np.logical_and(spikes2 >= ps, spikes2 < ps+cycle/ms))
